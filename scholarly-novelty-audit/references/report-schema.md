...
Every adverse report-level claim should link to stable evidence IDs. Use empty arrays or explicit `none found` explanations instead of omitting required negative findings. `run_manifest.runtime_environment` records the resolved Python, `jsonschema`, and `pypdf` versions. `report-attempt` replaces host-supplied strings with its own environment before validation and hashing; standalone `validate` checks historical records structurally without requiring the later machine to match. Bridge-policy status is also provenance: `DOCUMENTED_OVERRIDE` is not `CALIBRATION_DECLARED`, and the latter requires a structured dataset, method, and `preregistered=true`. Those fields are not independent verification of the calibration artifact or threshold.
...
Every graph-expansion call records its `provider`, `limit`, `exhausted`, `next_token`, provider/raw counts, and `possibly_truncated`. The expansion records all participating providers, per-provider endpoint observations, the preflight-derived observation-window status, and the provider union's negative-result scope. The validator requires `possibly_truncated == not exhausted`; an exhausted call cannot retain a continuation token. Non-exhausted calls require `LIMIT_REACHED` and prevent `COMPLETE`, while an exact-limit result may remain complete when the provider explicitly reports exhaustion. Every graph expansion records `provider_cutoff_applied=false`; historical expansions additionally require `temporal_recall_backstop=true` because final eligibility is resolved locally. Historical ordinary searches also require at least one SearchRun with the same backstop pairing.
