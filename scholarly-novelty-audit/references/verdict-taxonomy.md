...
Novelty Risk is not identical to classification. Search Protocol Coverage and Evidence Confidence can force a downgrade to `INCONCLUSIVE`; they must never be averaged into one score. Search Protocol Coverage describes execution of a bounded retrieval protocol, not demonstrated recall of every relevant work.
