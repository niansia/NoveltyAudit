...
def metric_case(annotation: dict, prediction: dict) -> dict:
    ...
    return {
        ...
        "bibliography_absent_gold": list(annotation.get("not_in_supplied_bibliography") or []),
...
def bibliography_absent_killer_recall_at_k(cases: Iterable[dict], k: int = 5) -> float:
    numerator = 0
    denominator = 0
    for case in cases:
        gold = set(case.get("bibliography_absent_gold") or [])
