...
11. For every multi-paper set, run the bundled `bridge` command. Its v0.3.2 default is a documented 500-citation operational guard supported by an exploratory sensitivity check across thresholds 50–1000; it is not universal field calibration or a performance claim. A sourced custom threshold is `DOCUMENTED_OVERRIDE`, not `CALIBRATION_DECLARED`. Use `CALIBRATION_DECLARED` only with structured calibration dataset and method provenance plus `preregistered=true`, and disclose that these fields are not independent verification of the calibration artifact or its support for the threshold. Inspect text before promoting a graph relation to an explicit extension, synthesis, benchmark, or combination bridge. Retain post-cutoff connections as `LANDSCAPE_BRIDGE`, but never let them change the historical verdict. Read [bridge evidence](references/bridge-evidence.md).
12. Run deterministic leave-one-out criticality sensitivity; validator recomputation must match the submitted results.
13. Before invoking helpers, verify Python 3.10+ and install `requirements.txt` when permitted. If dependencies cannot be installed, do not claim deterministic validation or Tier-2 PDF extraction; disclose the limitation and return `PARTIAL` or `INCONCLUSIVE`. Apply [verdict taxonomy](references/verdict-taxonomy.md), assemble the structured report, and run `report-attempt` with a fixed maximum of three attempts; `--input`, `--output`, and `--state` must be different paths. The command replaces any host-supplied runtime strings with its actual Python, `jsonschema`, and `pypdf` versions before validation and hashing. On `RETRY_REQUIRED`, repair the machine-bound output using only the disclosed validation failures and try again with distinct paths. On exhausted `PARTIAL`, stop, disclose the validation gaps, and cap the conclusion at `INCONCLUSIVE`; never export the invalid draft as a valid audit. Export the machine-bound report as Markdown, JSON, or HTML only after `COMPLETE`. Read [tooling](references/tooling.md).
