...
class SearchResult:
    ...
    @property
    def raw_returned_count(self) -> int:
        value = self.pagination.get("raw_returned_count")
        return int(value) if value is not None else self.returned_count
    ...
    def truncated(self) -> bool:
        ...
        return self.total_count is not None and offset + self.raw_returned_count < self.total_count
    ...
    def next_token(self) -> Any | None:
        ...
        offset = int(self.pagination.get("offset") or self.pagination.get("start") or 0)
        if self.total_count is not None and self.raw_returned_count > 0 and offset + self.raw_returned_count < self.total_count:
            return offset + self.raw_returned_count
        return None
