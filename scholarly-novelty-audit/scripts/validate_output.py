...
BIBLIOGRAPHY_STATUSES = {"IN_BIBLIOGRAPHY", "NOT_IN_BIBLIOGRAPHY", "BIBLIOGRAPHY_UNAVAILABLE"}
...
def graph_expansion_obligations(
    minimal_prior_sets: list[dict[str, Any]],
    graph_expansions: list[dict[str, Any]],
) -> tuple[set[tuple[str, str]], set[tuple[str, str]]]:
    """Return required MPS endpoint pairs and those lacking a complete expansion."""
    required = {
        tuple(sorted((paper_a, paper_b)))
        for mps in minimal_prior_sets
        for paper_a, paper_b in combinations(
            sorted({str(value) for value in mps.get("paper_ids") or []}), 2
        )
    }
    complete = {
        tuple(sorted(str(value) for value in expansion.get("paper_ids") or []))
        for expansion in graph_expansions
        if isinstance(expansion, dict)
        and expansion.get("status") == "COMPLETE"
        and len({str(value) for value in expansion.get("paper_ids") or []}) == 2
    }
    return required, required - complete


def graph_expansion_gap_marker(pair: tuple[str, str]) -> str:
    return f"GRAPH_EXPANSION_INCOMPLETE:{pair[0]}:{pair[1]}"


def validate_arxiv_page_history(run: dict[str, Any]) -> list[str]:
    """Validate that local cutoff filtering cannot corrupt raw arXiv traversal."""
    query_id = str(run.get("query_id") or "")
    pagination = run.get("pagination") or {}
    pages = pagination.get("pages")
    if not isinstance(pages, list) or not pages:
        return [f"query {query_id} arXiv pagination lacks auditable raw page history"]
    errors: list[str] = []
    expected_start = 0
    last_meta: dict[str, Any] = {}
    last_raw = last_eligible = 0
    for index, page in enumerate(pages):
        meta = page.get("pagination") if isinstance(page, dict) else None
        if not isinstance(meta, dict):
            errors.append(f"query {query_id} arXiv page {index} lacks pagination metadata")
            continue
        values = (meta.get("start"), meta.get("raw_returned_count"), meta.get("eligible_returned_count"))
        if any(not isinstance(value, int) or isinstance(value, bool) or value < 0 for value in values):
            errors.append(f"query {query_id} arXiv page {index} has invalid raw/eligible counts")
            continue
        start, raw_count, eligible_count = values
        if start != expected_start:
            errors.append(f"query {query_id} arXiv page {index} does not continue the raw offset")
        if eligible_count > raw_count or page.get("returned_count") != eligible_count:
            errors.append(f"query {query_id} arXiv page {index} eligible count disagrees with raw traversal")
        next_offset = meta.get("next")
        if next_offset is not None and (not isinstance(next_offset, int) or next_offset != start + raw_count or raw_count == 0):
            errors.append(f"query {query_id} arXiv page {index} has an invalid next raw offset")
        if index + 1 < len(pages) and next_offset is None:
            errors.append(f"query {query_id} arXiv page {index} claims exhaustion before a later page")
        expected_start = next_offset if isinstance(next_offset, int) else start + raw_count
        last_meta, last_raw, last_eligible = meta, raw_count, eligible_count
    stop_reason = run.get("saturation_stop_reason")
    if stop_reason == "PROVIDER_EXHAUSTED" and last_meta.get("next") is not None:
        errors.append(f"query {query_id} arXiv provider exhaustion conflicts with its next raw offset")
    if stop_reason == "NO_NEW_RESULTS" and last_raw > last_eligible:
        errors.append(f"query {query_id} arXiv filtered raw page cannot establish NO_NEW_RESULTS")
    return errors
...
def validate_report(report: dict[str, Any]) -> list[str]:
    ...
    errors: list[str] = []
    for field in ("schema_version", "audit_id", "generated_at", "candidate_ids", "run_manifest", "input", "author_bibliography", "claim_map", "verdict", "papers", "fulltext_acquisitions", "evidence", "top_killers", "minimal_prior_sets", "bridges", "landscape_bridges", "criticality_sensitivity", "ancestor_terms", "residual_novelty", "defensible_rewrite", "search", "excluded", "audit_log"):
    ...
    acquisition_items = report.get("fulltext_acquisitions") or []
    acquisition_ids = [str(item.get("id")) for item in acquisition_items]
    if len(acquisition_ids) != len(set(acquisition_ids)):
        errors.append("report contains duplicate full-text acquisition IDs")
    acquisitions = {str(item.get("id")): item for item in acquisition_items}
    ...
    for index, entry in enumerate(bibliography_entries):
        ...
        if matched in (None, ""):
            ...
            if not paper:
                ...
                errors.append(f"author bibliography entry {index} MANUAL match cannot establish machine-derived bibliography status")
            if verified_match:
    ...
    for acquisition_id, acquisition in acquisitions.items():
        paper_id = str(acquisition.get("paper_id") or "")
        if paper_id not in papers:
            errors.append(f"full-text acquisition {acquisition_id} references unknown paper {paper_id}")
        if acquisition.get("status") != "COMPLETE":
            errors.append(f"full-text acquisition {acquisition_id} is not complete")
    ...
    for evidence_id, item in evidence.items():
        ...
        if item.get("source_level") == "TIER_2_FULLTEXT":
            acquisition_id = str(item.get("acquisition_id") or "")
            acquisition = acquisitions.get(acquisition_id)
            if not acquisition:
                errors.append(f"Tier-2 evidence {evidence_id} lacks a recorded full-text acquisition")
            elif str(acquisition.get("paper_id")) != canonical_paper_id:
                errors.append(f"Tier-2 evidence {evidence_id} uses a full-text acquisition from another paper")
            else:
                acquired_at = parse_datetime(acquisition.get("retrieved_at"), f"full-text acquisition {acquisition_id}.retrieved_at")
                if acquired_at and retrieved_at:
                    acquired_compare, evidence_compare = comparable(acquired_at, retrieved_at)
                    if acquired_compare > evidence_compare:
                        errors.append(f"Tier-2 evidence {evidence_id} predates its full-text acquisition")
    ...
    for killer in report["top_killers"]:
        ...
        if killer.get("bibliography_status") not in BIBLIOGRAPHY_STATUSES:
            errors.append(f"top killer {paper_id} has invalid bibliography_status")
        expected_bibliography_status = (
            "BIBLIOGRAPHY_UNAVAILABLE" if bibliography.get("status") != "PROVIDED" else
            "IN_BIBLIOGRAPHY" if paper_id in bibliography_ids else "NOT_IN_BIBLIOGRAPHY"
        )
        if killer.get("bibliography_status") != expected_bibliography_status:
            errors.append(f"top killer {paper_id} bibliography_status disagrees with normalized author bibliography")
    ...
    _, incomplete_expansion_pairs = graph_expansion_obligations(
        computed_mps,
        report["search"].get("graph_expansions") or [],
    )
    expected_expansion_gaps = {
        graph_expansion_gap_marker(pair) for pair in incomplete_expansion_pairs
    }
    submitted_search_gaps = {str(value) for value in report["search"].get("gaps") or []}
    missing_expansion_gaps = expected_expansion_gaps - submitted_search_gaps
    if missing_expansion_gaps:
        errors.append(
            "incomplete graph expansion requires exact search-gap markers: "
            f"{sorted(missing_expansion_gaps)}"
        )
    ...
    if computed_size in {2, 3}:
        if incomplete_expansion_pairs and classification != "INCONCLUSIVE":
            errors.append(
                "graph expansion obligation incomplete for recomputed MPS endpoint pairs "
                f"{sorted(incomplete_expansion_pairs)}; incomplete graph expansion forces INCONCLUSIVE"
            )
        elif not incomplete_expansion_pairs and classification not in {"STRONG_COMPOSITION_RISK", "PLAUSIBLE_COMPOSITION_RISK", "FRAGMENTED_PRECEDENT"}:
    ...
    for index, run in enumerate(query_runs):
        required = ("query_id", "provider", "family", "query", "reason", "target_facets", "retrieved_at", "temporal_recall_backstop", "provider_cutoff_applied", "returned_count", "total_count", "truncated", "pagination", "corpus", "saturation_stop_reason", "paper_ids", "canonical_paper_ids", "status")
        ...
        if run.get("temporal_recall_backstop") is True and run.get("provider_cutoff_applied") is not False:
            errors.append(f"query {query_id} temporal recall backstop cannot apply the provider cutoff")
        ...
        if run.get("provider") == "arxiv" and run.get("status") == "ok":
    ...
    if report["input"].get("cutoff") and not any(
        run.get("temporal_recall_backstop") is True and run.get("provider_cutoff_applied") is False
        for run in query_runs if isinstance(run, dict)
    ):
        errors.append("at least one query run must be an unfiltered temporal recall backstop")
    ...
    if verdict.get("search_coverage") != derived_coverage["level"]:
        errors.append(f"verdict Search Protocol Coverage must equal deterministic SearchRun derivation: {derived_coverage['level']}")
