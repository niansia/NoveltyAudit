...
def command_expand_graph(args: argparse.Namespace) -> int:
    ...
    if isinstance(payload, dict):
        ...
        if result["status"] == "PARTIAL":
            marker = graph_expansion_gap_marker(tuple(sorted((str(args.paper_a), str(args.paper_b)))))
            search["gaps"] = list(dict.fromkeys(list(search.get("gaps") or []) + [marker]))
...
def command_fetch_fulltext(args: argparse.Namespace) -> int:
    papers = records(read_json(args.papers))
    if args.paper_id:
        requested = set(args.paper_id)
        papers = [paper for paper in papers if str(paper.get("id")) in requested]
        missing = sorted(requested - {str(paper.get("id")) for paper in papers})
        if missing:
            raise ValueError(f"requested paper IDs are absent from the papers file: {missing}")
    result = acquire_fulltexts(papers, args.output_dir, max_bytes=args.max_bytes)
...
def command_mps(args: argparse.Namespace) -> int:
    ...
    minimal_sets = solve_mps(papers, facets, max_size=args.max_size, strict=not args.non_strict, bridges=payload.get("bridges") if isinstance(payload, dict) else None)
    result = {
        "critical_facets": facets,
        "search_bound": {"max_papers": args.max_size, "larger_combinations_assessed": False},
        "minimal_prior_sets": minimal_sets,
        "no_result_explanation": None if minimal_sets else f"No qualifying evidence-bound prior set of size {args.max_size} or smaller was found. This does not assess larger combinations.",
...
def parser() -> argparse.ArgumentParser:
    ...
    fulltext = sub.add_parser("fetch-fulltext", help="acquire and extract provider-derived public full text")
    fulltext.add_argument("--papers", required=True)
    fulltext.add_argument("--paper-id", action="append", help="canonical paper ID; repeat to restrict acquisition")
    fulltext.add_argument("--output-dir", required=True)
    fulltext.add_argument("--manifest", required=True)
    fulltext.add_argument("--max-bytes", type=int, default=25_000_000)
    fulltext.set_defaults(func=command_fetch_fulltext)
