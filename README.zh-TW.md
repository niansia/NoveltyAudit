...
每個多篇 MPS 的端點 pair 都必須有 `COMPLETE` graph expansion。任何 call 回滿設定的 limit 都會標為可能截斷，使 expansion 成為 `PARTIAL/LIMIT_REACHED`；此時只能回報 `INCONCLUSIVE` 並留下 `GRAPH_EXPANSION_INCOMPLETE:<paper-a>:<paper-b>` gap，不能用「有界範圍內沒找到 bridge」支撐 `FRAGMENTED_PRECEDENT`。
...
若從 0.3.1 以前的版本升級，請先移除或封存舊的 `novelty-audit` skill 資料夾，再安裝 `scholarly-novelty-audit`；兩者並存可能造成重複或過期的 skill 觸發。
