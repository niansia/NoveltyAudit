...
- Co-citation cannot strengthen a verdict without complete citation counts and a documented field-calibrated or sensitivity-checked operational threshold; post-cutoff graph routes remain visible only as non-adverse landscape bridges.
...
- Citation expansion runs a pre-retrieval observation-window diagnostic, unions available OpenAlex and Semantic Scholar backward routes and shared-namespace forward routes, merges third-paper sources into the candidate pool, and records provider-attributed failures or exhausted limits. Every endpoint pair in every recomputed multi-paper MPS requires a complete, non-truncated expansion; otherwise the verdict is capped at `INCONCLUSIVE` with a deterministic search-gap marker. Complete zero results must expose per-provider endpoint observations, the numeric observation window and status, historical/landscape routing, and a validator-recomputed negative-result scope.
...
Current evidence is explicitly counted in [empirical validation status](empirical-status.md). As of 2026-08-27, all 82 licensed annotated cases have entered a deterministic no-LLM bridge base-rate study: 23 have at least two deterministically detected reviewer-named priors, 18 have complete endpoint/pair coverage, and 4 of those 18 contain a pre-cutoff co-citation bridge. The first figure is a detected rate, not a formal statistical lower bound or composition-objection prevalence; extracted links have not yet been independently precision-audited. The 4/18 case-level exact interval is wide, while pair-level rates are descriptive because pairs cluster within cases. This establishes a sample base rate and provider/observation-window limitations, not end-to-end audit quality. Zero cases have completed the full reviewer-grounded claim-to-evidence pipeline; Recall@5, MRR, calibration, and reviewer prediction remain unmeasured.
