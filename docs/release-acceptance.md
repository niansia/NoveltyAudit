...
- Citation expansion runs a pre-retrieval maturity diagnostic, unions available OpenAlex and Semantic Scholar backward routes and shared-namespace forward routes, merges third-paper sources into the candidate pool, and records provider-attributed failures or exhausted limits. Every endpoint pair in every recomputed multi-paper MPS requires a complete, non-truncated expansion; otherwise the verdict is capped at `INCONCLUSIVE` with a deterministic search-gap marker. Complete zero results must expose per-provider endpoint observations, the numeric observation window and status, historical/landscape routing, and a validator-recomputed negative-result scope.
